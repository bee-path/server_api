...
class Flight(object):
    ...
    def __init__(self,points=None):
        if len(points)==0:
            ...
            self.atts[4]=np.linalg.norm(self.atts[2][-1][:-1]-self.atts[2][0][:-1]) # exclude time!
...
def main(GPSpoints,R1=1.,R2=10.,rand=False):
    ...
    Trac=rectangularmodel(stopormove_v(latlon_2_UTM(GPSpoints,origin,zone,time=True),R1,t_min=1,v_min=0.33),R2) # with velocity
    return Trac
