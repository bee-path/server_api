...
### Inner level (related with user-sessions): 
Only allow POST and GET methods. All POSTS are blocked if the user and the session are not active, and the session does not need to be specified, as long as there is at least one active session.
#### Found_target: 
